...
func TestGatewayRoutesGroupModelAllowlistMountedOnEveryGatewayRoute(t *testing.T) {
	...
	rootHelper := regexp.MustCompile(regexp.QuoteMeta(`r.Handle(method, path, limit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), callAuditHandler, groupModelAllowlist, compositeTarget, requireGroupAnthropic, handler)`))
	...
	codexDirect := regexp.MustCompile(regexp.QuoteMeta(`codexDirect.Use(bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), callAuditHandler, groupModelAllowlist, compositeTarget, requireGroupAnthropic)`))
...
func TestGatewayRoutesGroupModelAllowlistCoversRootAliasRoutes(t *testing.T) {
	...
	paths := []struct {
		...
		{http.MethodPost, "/completions", `{"prompt":"hi"}`},
		{http.MethodPost, "/openai/v1/completions", `{"prompt":"hi"}`},
		{http.MethodPost, "/completions", `{"model":"gpt-4.1","prompt":"hi"}`},
		{http.MethodPost, "/v1/completions", `{"model":"gpt-4.1","prompt":"hi"}`},
		{http.MethodPost, "/api/v1/messages", `{"model":"gpt-4.1","prompt":"hi"}`},
		{http.MethodPost, "/api/v1/completions", `{"model":"gpt-4.1","prompt":"hi"}`},
		{http.MethodPost, "/claude/v1/completions", `{"model":"gpt-4.1","prompt":"hi"}`},
		{http.MethodPost, "/openai/v1/responses", `{"model":"gpt-4.1","prompt":"hi"}`},
		{http.MethodPost, "/openai/v1/completions", `{"model":"gpt-4.1","prompt":"hi"}`},
		{http.MethodPost, "/antigravity/api/v1/messages", `{"model":"gpt-4.1","prompt":"hi"}`},
		{http.MethodPost, "/gemini-cli/api/v1/messages", `{"model":"gpt-4.1","prompt":"hi"}`},
