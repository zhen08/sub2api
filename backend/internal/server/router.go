...
func SetupRouter(
	...
	callAudit middleware2.CallAuditMiddleware,
	callAuditRuntime *callaudit.Runtime,
	...
	// 注册路由
	registerRoutes(r, handlers, jwtAuth, optionalJWTAuth, adminAuth, apiKeyAuth, callAudit, callAuditRuntime, auditLog, stepUpAuth, apiKeyService, subscriptionService, opsService, settingService, compositeResolver, cfg, redisClient)
...
func registerRoutes(
	...
	// 通用路由（健康检查、状态等）
	routes.RegisterCommonRoutes(r, callAuditRuntime)
	...
	routes.RegisterCallAuditAdminRoutes(v1, callAuditRuntime, gin.HandlerFunc(adminAuth))
	routes.RegisterGatewayRoutes(r, h, apiKeyAuth, callAudit, apiKeyService, subscriptionService, opsService, settingService, compositeResolver, cfg)
