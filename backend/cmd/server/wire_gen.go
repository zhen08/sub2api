...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	runtime := callaudit.NewRuntime(configConfig)
	httpUpstream := repository.ProvideHTTPUpstream(configConfig, runtime)
	...
	yunMoStarIntegrationService := service.NewYunMoStarIntegrationService(client, apiKeyService, configConfig)
	yunMoStarIntegrationHandler := admin.NewYunMoStarIntegrationHandler(yunMoStarIntegrationService)
	upstreamBillingProbeService := service.ProvideUpstreamBillingProbeService(accountRepository, accountTestService, settingService, leaderLockCache, db)
	adminHandlers := handler.ProvideAdminHandlers(dashboardHandler, adminUserHandler, groupHandler, accountHandler, adminAnnouncementHandler, dataManagementHandler, backupHandler, oAuthHandler, openAIOAuthHandler, geminiOAuthHandler, antigravityOAuthHandler, grokOAuthHandler, cnProviderHandler, proxyHandler, adminRedeemHandler, promoHandler, settingHandler, opsHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, userAttributeHandler, errorPassthroughHandler, tlsFingerprintProfileHandler, pluginHandler, adminAPIKeyHandler, scheduledTestHandler, channelHandler, channelMonitorHandler, channelMonitorRequestTemplateHandler, contentModerationHandler, promptAdminHandler, paymentHandler, affiliateHandler, complianceHandler, auditLogHandler, yunMoStarIntegrationHandler, upstreamBillingProbeService, ollamaCloudUsageService)
	...
	callAuditMiddleware := middleware.NewCallAuditMiddleware(runtime)
	...
	engine := server.ProvideRouter(configConfig, handlers, jwtAuthMiddleware, optionalJWTAuthMiddleware, adminAuthMiddleware, apiKeyAuthMiddleware, callAuditMiddleware, runtime, auditLogMiddleware, stepUpAuthMiddleware, apiKeyService, subscriptionService, opsService, settingService, compositeRouteResolver, redisClient)
	httpServer := server.ProvideHTTPServer(configConfig, engine)
	mainPluginAccountDirectoryBinding := bindPluginAccountDirectory(pluginManager, openAIGatewayService)
	...
	application := &Application{
		...
		PluginAccountDirectoryBinding: mainPluginAccountDirectoryBinding,
		Cleanup:                       v,
	}
