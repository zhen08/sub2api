...
func RegisterGatewayRoutes(
	...
	rootRoute := func(method, path string, limit gin.HandlerFunc, handler gin.HandlerFunc) {
		r.Handle(method, path, limit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), callAuditHandler, groupModelAllowlist, compositeTarget, requireGroupAnthropic, handler)
	}
	...
	codexDirect.Use(bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), callAuditHandler, groupModelAllowlist, compositeTarget, requireGroupAnthropic)
	{
	...
	rootRoute(http.MethodPost, "/completions", bodyLimit, completionsHandler)
...
func legacyCompletionsHandler(next gin.HandlerFunc) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if _, exists := mapped["model"]; !exists {
			// The compatibility default is introduced after ingress validation;
			// enforce the group policy before forwarding this implicit model.
			const defaultModel = "claude-3-5-sonnet-20241022"
			if key, ok := middleware.GetAPIKeyFromContext(c); ok && key != nil && key.Group != nil && key.Group.ModelAllowlistEnabled() && !key.Group.ModelAllowlist.Allows(defaultModel) {
				service.MarkOpsClientBusinessLimited(c, service.OpsClientBusinessLimitedReasonLocalModelConfiguration)
				middleware.MarkIngressRejected(c, middleware.IngressRejectModelNotAllowed)
				middleware.AnthropicErrorWriter(c, http.StatusNotFound, "Model \""+defaultModel+"\" is not available for this group")
				c.Abort()
				return
			}
			mapped["model"] = json.RawMessage(`"` + defaultModel + `"`)
		}
