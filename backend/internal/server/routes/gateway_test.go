...
func newGatewayRoutesTestRouterWithConfig(cfg *config.Config, platform ...string) *gin.Engine {
	...
	return newGatewayRoutesTestRouterWithAuditAndConfig(groupPlatform, nil, cfg)
}

func newGatewayRoutesTestRouterWithAudit(groupPlatform string, callAudit servermiddleware.CallAuditMiddleware) *gin.Engine {
	return newGatewayRoutesTestRouterWithAuditAndConfig(groupPlatform, callAudit, &config.Config{})
}

func newGatewayRoutesTestRouterWithAuditAndConfig(groupPlatform string, callAudit servermiddleware.CallAuditMiddleware, cfg *config.Config) *gin.Engine {
...
func TestEveryGatewayPOSTHasExplicitCallAuditPolicy(t *testing.T) {
	...
	for _, route := range router.Routes() {
		...
		intentionallyExcluded := strings.Contains(path, "/count_tokens") ||
			...
			strings.HasSuffix(path, "/tts") || strings.HasSuffix(path, "/stt") ||
			strings.HasSuffix(path, "/custom-voices") ||
			strings.HasSuffix(path, "/alpha/search") ||
			strings.HasSuffix(path, "/web_search") || strings.HasSuffix(path, "/x_search")
